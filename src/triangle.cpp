...
{
	throw new std::runtime_error("Could not triangulate domain.");
  //exit(status);
}

#ifdef ANSI_DECLARATORS
#ifdef _DEBUG 
	VOID *trimalloc(int size, char *szFile, unsigned int nLineNo )
#else
...
#ifdef _DEBUG
	memptr = (VOID *)_malloc_dbg((unsigned int) size, _NORMAL_BLOCK,szFile,nLineNo);
#else
	memptr = (VOID *)malloc((unsigned int) size);
#endif
	if (memptr == (VOID *) NULL) 
	{
		//odprintf("Error:  Out of memory.\n");
		throw new std::runtime_error("Could not triangulate domain:\nOut of memory.");
		//triexit(1);
	}
	return(memptr);
...
#ifdef _DEBUG
#define trimalloc(s) trimalloc( s, __FILE__, __LINE__ );
#endif
...
#ifdef CDT_ONLY
#ifdef REDUCED
  odprintf("triangle [-pAcjevngBPNEIOXzo_lQVh] input_file\n");
#else /* not REDUCED */
  odprintf("triangle [-pAcjevngBPNEIOXzo_iFlCQVh] input_file\n");
#endif /* not REDUCED */
#else /* not CDT_ONLY */
#ifdef REDUCED
  odprintf("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__lQVh] input_file\n");
#else /* not REDUCED */
  odprintf("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__iFlsCQVh] input_file\n");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */

  odprintf("    -p  Triangulates a Planar Straight Line Graph (.poly file).\n");
#ifndef CDT_ONLY
  odprintf("    -r  Refines a previously generated mesh.\n");
  odprintf(
    "    -q  Quality mesh generation.  A minimum angle may be specified.\n");
  odprintf("    -a  Applies a maximum triangle area constraint.\n");
  odprintf("    -u  Applies a user-defined triangle constraint.\n");
#endif /* not CDT_ONLY */
  odprintf(
    "    -A  Applies attributes to identify triangles in certain regions.\n");
  odprintf("    -c  Encloses the convex hull with segments.\n");
#ifndef CDT_ONLY
  odprintf("    -D  Conforming Delaunay:  all triangles are truly Delaunay.\n");
#endif /* not CDT_ONLY */
/*
  odprintf("    -w  Weighted Delaunay triangulation.\n");
  odprintf("    -W  Regular triangulation (lower hull of a height field).\n");
*/
  odprintf("    -j  Jettison unused vertices from output .node file.\n");
  odprintf("    -e  Generates an edge list.\n");
  odprintf("    -v  Generates a Voronoi diagram.\n");
  odprintf("    -n  Generates a list of triangle neighbors.\n");
  odprintf("    -g  Generates an .off file for Geomview.\n");
  odprintf("    -B  Suppresses output of boundary information.\n");
  odprintf("    -P  Suppresses output of .poly file.\n");
  odprintf("    -N  Suppresses output of .node file.\n");
  odprintf("    -E  Suppresses output of .ele file.\n");
  odprintf("    -I  Suppresses mesh iteration numbers.\n");
  odprintf("    -O  Ignores holes in .poly file.\n");
  odprintf("    -X  Suppresses use of exact arithmetic.\n");
  odprintf("    -z  Numbers all items starting from zero (rather than one).\n");
  odprintf("    -o2 Generates second-order subparametric elements.\n");
#ifndef CDT_ONLY
  odprintf("    -Y  Suppresses boundary segment splitting.\n");
  odprintf("    -S  Specifies maximum number of added Steiner points.\n");
#endif /* not CDT_ONLY */
#ifndef REDUCED
  odprintf("    -i  Uses incremental method, rather than divide-and-conquer.\n");
  odprintf("    -F  Uses Fortune's sweepline algorithm, rather than d-and-c.\n");
#endif /* not REDUCED */
  odprintf("    -l  Uses vertical cuts only, rather than alternating cuts.\n");
#ifndef REDUCED
#ifndef CDT_ONLY
  odprintf(
    "    -s  Force segments into mesh by splitting (instead of using CDT).\n");
#endif /* not CDT_ONLY */
  odprintf("    -C  Check consistency of final mesh.\n");
#endif /* not REDUCED */
  odprintf("    -Q  Quiet:  No terminal output except errors.\n");
  odprintf("    -V  Verbose:  Detailed information on what I'm doing.\n");
  odprintf("    -h  Help:  Detailed instructions for Triangle.\n");
  triexit(0);
}
...
void info()
{
  ...
  odprintf(
"A Two-Dimensional Quality Mesh Generator and Delaunay Triangulator.\n");
  odprintf("Version 1.6\n\n");
  odprintf(
"Copyright 1993, 1995, 1997, 1998, 2002, 2005 Jonathan Richard Shewchuk\n");
  odprintf("2360 Woolsey #H / Berkeley, California 94705-1927\n");
  odprintf("Bugs/comments to jrs@cs.berkeley.edu\n");
  odprintf(
"Created as part of the Quake project (tools for earthquake simulation).\n");
  odprintf(
"Supported in part by NSF Grant CMS-9318163 and an NSERC 1967 Scholarship.\n");
  odprintf("There is no warranty whatsoever.  Use at your own risk.\n");
#ifdef SINGLE
  odprintf("This executable is compiled for single precision arithmetic.\n\n\n");
#else /* not SINGLE */
  odprintf("This executable is compiled for double precision arithmetic.\n\n\n");
#endif /* not SINGLE */
  odprintf(
...
"and possibly a .poly file and an .area file as well.  The formats of these\n"
  ...
  odprintf("files are described below.\n\n");
  odprintf("Command Line Switches:\n\n");
  odprintf(
...
"        triunsuitable().  In either case, the -u switch causes the user-\n");
  odprintf("        defined test to be applied to every triangle.\n");
  odprintf(
...
"        .poly file to be written containing the boundary edges of the mesh\n"
  ...
  odprintf("        (useful if no .poly file was read).\n");
  odprintf(
...
"    -g  Outputs the mesh to an Object File Format (.off) file, suitable for\n"
  ...
  odprintf("        viewing with the Geometry Center's Geomview package.\n");
  odprintf(
...
"        to maintain constraining segments on later refinements of the mesh.\n"
  ...
  odprintf("    -N  No output .node file.\n");
  odprintf("    -E  No output .ele file.\n");
  odprintf(
...
"        using a .node file for input, because no .node file is written, so\n"
  ...
  odprintf("        there is no record of any added Steiner points.\n");
  odprintf("    -O  No holes.  Ignores the holes in the .poly file.\n");
  odprintf(
...
"        improvement in speed and creates the possibility that Triangle will\n"
  ...
  odprintf("        fail to produce a valid mesh.  Not recommended.\n");
  odprintf(
...
"        always adds enough vertices to ensure that all the segments of the\n"
  ...
  odprintf("        PLSG are recovered, ignoring the limit if necessary.\n");
  odprintf(
...
"        construct a Delaunay triangulation.  Try it if the divide-and-\n");
  odprintf("        conquer algorithm fails.\n");
  odprintf(
...
"        triangulation.  Warning:  does not use exact arithmetic for all\n");
  odprintf("        calculations.  An exact result is not guaranteed.\n");
  odprintf(
...
"        prints so much that Triangle runs much more slowly.  `-VVVV' gives\n"
  ...
  odprintf("        information only a debugger could love.\n");
  odprintf("    -h  Help:  Displays these instructions.\n");
  odprintf("\n");
  odprintf("Definitions:\n");
  odprintf("\n");
  odprintf(
...
"  inside the circumscribing circle (circle that passes through all three\n");
  odprintf("  vertices) of any triangle in the triangulation.\n\n");
  odprintf(
...
"  to some input vertex than to any other input vertex.  The Voronoi diagram\n"
  ...
  odprintf("  is a geometric dual of the Delaunay triangulation.\n\n");
  odprintf(
...
"  Segments may intersect each other only at their endpoints.  The file\n");
  odprintf("  format for PSLGs (.poly files) is described below.\n\n");
  odprintf(
...
"  cover the convex hull of the PSLG, or it might cover only a segment-\n");
  odprintf("  bounded region (e.g. a polygon).\n\n");
  odprintf(
...
"  The new vertices are frequently necessary to maintain the Delaunay\n");
  odprintf("  property while ensuring that every segment is represented.\n\n");
  odprintf(
...
"  triangulation of a PSLG whose triangles are constrained Delaunay.  New\n");
  odprintf("  vertices may appear, and input segments may be subdivided into\n");
  odprintf(
...
"  by the -q switch are usually CCDTs, but can be made conforming Delaunay\n");
  odprintf("  with the -D switch.\n\n");
  odprintf("File Formats:\n\n");
  odprintf(
...
"  another program, use the -z switch if you wish to number objects from\n");
  odprintf("  zero.)  Examples of these file formats are given below.\n\n");
  odprintf("  .node files:\n");
  odprintf(
...
"    -a, -u, -D, or -s is selected, each new Steiner point added to the mesh\n"
  ...
  odprintf("    has attributes assigned to it by linear interpolation.\n\n");
  odprintf(
...
"    .node file produced by Triangle contains boundary markers in the last\n");
  odprintf("    column unless they are suppressed by the -B switch.\n\n");
  odprintf("  .ele files:\n");
  odprintf(
...
"    around each triangle.  (The remaining nodes, if any, depend on the type\n"
  ...
  odprintf("    of finite element used.)\n\n");
  odprintf(
...
"    among nearby triangles as the triangulation is refined.  Attributes do\n"
  ...
  odprintf("    not diffuse across segments, so attributes used to identify\n");
  odprintf("    segment-bounded regions remain intact.\n\n");
  odprintf(
...
"    Following lines:  <vertex #> <x> <y> [attributes] [boundary marker]\n");
  odprintf("    One line:  <# of segments> <# of boundary markers (0 or 1)>\n");
  odprintf(
"    Following lines:  <segment #> <endpoint> <endpoint> [boundary marker]\n");
  odprintf("    One line:  <# of holes>\n");
  odprintf("    Following lines:  <hole #> <x> <y>\n");
  odprintf(
...
"    means that you must include its endpoints in the vertex list.  Each\n");
  odprintf("    segment, like each point, may have a boundary marker.\n\n");
  odprintf(
...
"    segment itself is also eaten.  Do not place a hole directly on a\n");
  odprintf("    segment; if you do, Triangle chooses one side of the segment\n");
  odprintf("    arbitrarily.\n\n");
  odprintf(
...
"    simultaneously and wish to assign an attribute to some region without\n");
  odprintf("    imposing an area constraint, use a negative maximum area.\n\n");
  odprintf(
...
"    the segment, and you'll have a needle-sharp triangle in your output--or\n"
  ...
  odprintf("    a lot of tiny triangles if you're generating a quality mesh.\n");
  odprintf("\n");
  odprintf(
...
"    mesh, and don't want segments to be missing in later triangulations.\n");
  odprintf("\n");
  odprintf("  .area files:\n");
  odprintf("    First line:  <# of triangles>\n");
  odprintf("    Following lines:  <triangle #> <maximum area>\n");
  odprintf("\n");
  odprintf(
...
"    triangle may be left unconstrained by assigning it a negative maximum\n");
  odprintf("    area.\n\n");
  odprintf("  .edge files:\n");
  odprintf("    First line:  <# of edges> <# of boundary markers (0 or 1)>\n");
  odprintf(
...
"    infinite ray with only one endpoint.  For these edges, a different\n");
  odprintf("    format is used:\n\n");
  odprintf("        <edge #> <endpoint> -1 <direction x> <direction y>\n\n");
  odprintf(
"    The `direction' is a floating-point vector that indicates the direction\n"
);
  odprintf("    of the infinite ray.\n\n");
  odprintf("  .neigh files:\n");
  odprintf(
...
"  finite element mesh.  You can prevent boundary markers from being written\n"
  ...
  odprintf("  into files produced by Triangle by using the -B switch.\n\n");
  odprintf(
"  The boundary marker associated with each segment in an output .poly file\n"
);
  odprintf("  and each edge in an output .edge file is chosen as follows:\n");
  odprintf(
...
"      (even the boundary of a hole), then the edge is assigned the marker\n");
  odprintf("      one (1).\n");
  odprintf("    - Otherwise, the edge is assigned the marker zero (0).\n");
  odprintf(
"  The boundary marker associated with each vertex in an output .node file\n");
  odprintf("  is chosen as follows:\n");
  odprintf(
...
"      vertex is assigned the same marker.  If the vertex lies on several\n");
  odprintf("      such segments, one of the markers is chosen arbitrarily.\n");
  odprintf(
"    - Otherwise, if the vertex occurs on a boundary of the triangulation,\n");
  odprintf("      then the vertex is assigned the marker one (1).\n");
  odprintf("    - Otherwise, the vertex is assigned the marker zero (0).\n");
  odprintf("\n");
  odprintf(
...
"  all) in your input files.  In the output files, all boundary vertices,\n");
  odprintf("  edges, and segments will be assigned the value one.\n\n");
  odprintf("Triangulation Iteration Numbers:\n\n");
  odprintf(
...
"  triangulation, and output the files mesh.4.node, mesh.4.ele, and\n");
  odprintf("  mesh.4.poly.  Files with no iteration number are treated as if\n");
  odprintf(
...
"  points, a .node file is written.  This can be quite convenient for\n");
  odprintf("  computing CDTs or quality meshes.)\n\n");
  odprintf("Examples of How to Use Triangle:\n\n");
  odprintf(
...
"  mesh to object.1.node and object.1.ele.  Each segment may be broken up\n");
  odprintf("  into multiple subsegments; these are written to object.1.poly.\n");
  odprintf("\n");
  odprintf(
...
"    # A box with eight vertices in 2D, no attributes, one boundary marker.\n"
  ...
  odprintf("    8 2 0 1\n");
  odprintf("     # Outer box has these vertices:\n");
  odprintf("     1   0 0   0\n");
  odprintf("     2   0 3   0\n");
  odprintf("     3   3 0   0\n");
  odprintf("     4   3 3   33     # A special marker for this vertex.\n");
  odprintf("     # Inner square has these vertices:\n");
  odprintf("     5   1 1   0\n");
  odprintf("     6   1 2   0\n");
  odprintf("     7   2 1   0\n");
  odprintf("     8   2 2   0\n");
  odprintf("    # Five segments with boundary markers.\n");
  odprintf("    5 1\n");
  odprintf("     1   1 2   5      # Left side of outer box.\n");
  odprintf("     # Square hole has these segments:\n");
  odprintf("     2   5 7   0\n");
  odprintf("     3   7 8   0\n");
  odprintf("     4   8 6   10\n");
  odprintf("     5   6 5   0\n");
  odprintf("    # One hole in the middle of the inner square.\n");
  odprintf("    1\n");
  odprintf("     1   1.5 1.5\n");
  odprintf("\n");
  odprintf(
...
"  other vertices but 4 have been marked to indicate that they lie on a\n");
  ...
  odprintf("      11    3   1.5    1\n");
  odprintf("      12    1.5   3    1\n");
  odprintf("    # Generated by triangle -pqc box.poly\n");
  odprintf("\n");
  odprintf("  Here is the output file `box.1.ele', with twelve triangles.\n");
  odprintf("\n");
  odprintf("    12  3  0\n");
  odprintf("       1     5   6   9\n");
  odprintf("       2    10   3   7\n");
...
"  newly added vertices.  Note also that <# of vertices> is set to zero to\n");
  odprintf("  indicate that the vertices should be read from the .node file.\n");
  odprintf("\n");
  odprintf("    0  2  0  1\n");
  odprintf("    12  1\n");
  odprintf("       1     1   9     5\n");
  ...
  odprintf("    # Generated by triangle -pqc box.poly\n");
  odprintf("\n");
  odprintf("Refinement and Area Constraints:\n");
  odprintf("\n");
  odprintf(
...
"  consider instead using the -u switch and writing a C procedure that\n");
  odprintf("  determines which triangles are too large.\n\n");
  odprintf(
...
"  the nodes at the corners of an element; the midpoint nodes on each edge\n");
  odprintf("  are discarded before the mesh is refined.\n\n");
  odprintf(
...
"  are typically based on a posteriori error estimates resulting from a\n");
  odprintf("  finite element simulation on that mesh.\n\n");
  odprintf(
...
"  x.2' creates a sequence of successively finer meshes x.1, x.2, and x.3,\n");
  odprintf("  suitable for multigrid.\n\n");
  odprintf("Convex Hulls and Mesh Boundaries:\n\n");
  odprintf(
...
"  There are faster algorithms for finding a two-dimensional convex hull\n");
  odprintf("  than triangulation, of course, but this one comes for free.\n\n");
  odprintf(
...
"  If you also use the -p switch, the output .poly file contains all the\n");
  odprintf("  segments from the input .poly file as well.\n\n");
  odprintf("Voronoi Diagrams:\n\n");
  odprintf(
...
"  rays.  (The choice of filenames makes it easy to run the set of Voronoi\n");
  odprintf("  vertices through Triangle, if so desired.)\n\n");
  odprintf(
...
"  construct a conforming Delaunay triangulation instead of a CCDT, so the\n");
  odprintf("  Voronoi diagram will be valid.\n\n");
  odprintf("Mesh Topology:\n\n");
  odprintf(
...
"  output files with the recollection that the Delaunay triangulation and\n");
  odprintf("  the Voronoi diagram are planar duals.\n\n");
  odprintf(
...
"  vertex j of the corresponding .v.node file.  Voronoi cell k is the dual\n");
  odprintf("  of vertex k of the corresponding .node file.\n\n");
  odprintf(
...
"  respectively.  To find which Voronoi cells are adjacent to each other,\n");
  odprintf("  just read the list of Delaunay edges.\n\n");
  odprintf(
...
"  Voronoi vertices attached to it.  You might find this more convenient\n");
  odprintf("  than the .v.edge file.\n\n");
  odprintf("Quadratic Elements:\n\n");
  odprintf(
...
"  and sixth nodes appearing opposite the first, second, and third corners\n");
  odprintf("  respectively.\n\n");
  odprintf("Domains with Small Angles:\n\n");
  odprintf(
...
"  is reduced to memory precision.  Triangle does this if it is compiled\n");
  odprintf("  correctly.  See the makefile for details.\n\n");
  odprintf(
...
"  inputs most likely to cause errors if you use the -X switch.  Hence, the\n"
  ...
  odprintf("  -X switch is not recommended.\n\n");
  odprintf(
...
"  files.)  Similarly, exact arithmetic is not used to compute the vertices\n"
  ...
  odprintf("  of the Voronoi diagram.\n\n");
  odprintf(
...
"  being performed exactly, while overflow typically causes a floating\n");
  odprintf("  exception.\n\n");
  odprintf("Calling Triangle from Another Program:\n\n");
  odprintf("  Read the file triangle.h for details.\n\n");
  odprintf("Troubleshooting:\n\n");
  odprintf("  Please read this section before mailing me bugs.\n\n");
  odprintf("  `My output mesh has no triangles!'\n\n");
  odprintf(
...
"    have placed a hole badly, thereby eating all your triangles.  To test\n");
  odprintf("    these possibilities, try again with the -c and -O switches.\n");
  odprintf(
"    Alternatively, all your input vertices may be collinear, in which case\n"
);
  odprintf("    you can hardly expect to triangulate them.\n\n");
  odprintf("  `Triangle doesn't terminate, or just crashes.'\n\n");
  odprintf(
...
"    between 6247133 and 6247134, you're not leaving much floating-point\n");
  odprintf("    precision for Triangle to work with.\n\n");
  odprintf(
...
"    above.)  Again, I recommend compiling Triangle for double (rather\n");
  odprintf("    than single) precision arithmetic.\n\n");
  odprintf(
...
"    extremely tiny triangles, or it might fail because of insufficient\n");
  odprintf("    floating-point precision.\n\n");
  odprintf(
...
"    don't use the -X switch.  If you still have problems, please report the\n"
  ...
  odprintf(
"  `Triangle executes without incident, but when I look at the resulting\n");
  odprintf("  Voronoi diagram, it has overlapping edges or other geometric\n");
  odprintf("  inconsistencies.'\n");
  odprintf("\n");
  odprintf(
...
"    holes, and you use the -D switch to construct a conforming Delaunay\n");
  odprintf("    triangulation (instead of a CDT or CCDT).\n\n");
  odprintf(
...
"  doesn't, and you'll have to connect each adjacent pair of vertices with a\n"
  ...
  odprintf("  separate segment.  If you don't like it, tough.\n\n");
  odprintf(
...
"  endpoints, try not to let the intersections fall extremely close to PSLG\n"
  ...
  odprintf("  vertices or each other.\n\n");
  odprintf(
...
"  triangles constrained Delaunay?  Triangle's Delaunay refinement algorithm\n"
  ...
  odprintf("  assumes that it starts with a CDT.\n\n");
  odprintf("Show Me:\n\n");
  odprintf(
...
"  you have the X Windows system.  Sorry, Microsoft Windows users.\n");
  odprintf("\n");
  odprintf("Triangle on the Web:\n");
  odprintf("\n");
  odprintf("  To see an illustrated version of these instructions, check out\n");
  odprintf("\n");
  odprintf("    http://www.cs.cmu.edu/~quake/triangle.html\n");
  odprintf("\n");
  odprintf("A Brief Plea:\n");
  odprintf("\n");
  odprintf(
...
"  you.  Also, I can put you on a list to receive email whenever a new\n");
  odprintf("  version of Triangle is available.\n\n");
  odprintf(
...
"  Schachter, Micha Sharir, Peter W. Shor, Daniel D. Sleator, Jorge Stolfi,\n"
  ...
  odprintf("  Robert E. Tarjan, Alper Ungor, Christopher J. Van Wyk, Noel J.\n");
  odprintf(
"  Walkington, and Binhai Zhu.  See the comments at the beginning of the\n");
  odprintf("  source code for references.\n\n");
  triexit(0);
}
...
void internalerror()
{
  odprintf("  Please report this bug to jrs@cs.berkeley.edu\n");
  odprintf("  Include the message above, your input data set, and the exact\n");
  odprintf("    command line you used to run Triangle.\n");
  throw new std::runtime_error("Could not triangulate domain:\nInternal Error.");
  //triexit(1);
}
...
#define STARTINDEX 1
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            if (b->maxarea <= 0.0) 
			{
              odprintf("Error:  Maximum area must be greater than zero.\n");
  ...
  if (b->weighted && (b->poly || b->quality)) {
    b->weighted = 0;
    if (!b->quiet) {
      odprintf("Warning:  weighted triangulations (-w, -W) are incompatible\n");
      odprintf("  with PSLGs (-p) and meshing (-q, -a, -u).  Weights ignored.\n"
             );
    }
  }
  if (b->jettison && b->nonodewritten && !b->quiet) {
    odprintf("Warning:  -j and -N switches are somewhat incompatible.\n");
    odprintf("  If any vertices are jettisoned, you will need the output\n");
    odprintf("  .node file to reconstruct the new node indices.");
  }
...

{
  ...
  odprintf("triangle x%lx with orientation %d:\n", (unsigned long long) t->tri,
         t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == m->dummytri) {
    odprintf("    [0] = Outer space\n");
  } else {
  ...
  if (printvertex == (vertex) NULL)
    odprintf("    Origin[%d] = NULL\n", (t->orient + 1) % 3 + 3);
  else
    odprintf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (unsigned long long) printvertex,
           printvertex[0], printvertex[1]);
  dest(*t, printvertex);
  if (printvertex == (vertex) NULL)
    odprintf("    Dest  [%d] = NULL\n", (t->orient + 2) % 3 + 3);
  else
    odprintf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (unsigned long long) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    odprintf("    Apex  [%d] = NULL\n", t->orient + 3);
  else
    odprintf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
...

{
  ...
  odprintf("subsegment x%lx with orientation %d and mark %d:\n",
         (unsigned long long) s->ss, s->ssorient, mark(*s));
  sdecode(s->ss[0], printsh);
  if (printsh.ss == m->dummysub) {
    odprintf("    [0] = No subsegment\n");
  } else {
    odprintf("    [0] = x%lx  %d\n", (unsigned long long) printsh.ss,
           printsh.ssorient);
  }
  ...
  if (printvertex == (vertex) NULL)
    odprintf("    Segment origin[%d] = NULL\n", 4 + s->ssorient);
  else
    odprintf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (unsigned long long) printvertex,
           printvertex[0], printvertex[1]);
  segdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    odprintf("    Segment dest  [%d] = NULL\n", 5 - s->ssorient);
  else
    odprintf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
...
#if defined(PLATFORM_WINDOWS) && !defined(BUILD_64)
...

{
  ...
  b->noexact = 0;
  if (!b->quiet) {
    odprintf("  Checking consistency of mesh...\n");
  }
  horrors = 0;
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (oppotri.tri != m->dummytri) {
        ...
        if ((triangleloop.tri != oppooppotri.tri)
          ...
          odprintf("  !! !! Asymmetric triangle-triangle bond:\n");
          if (triangleloop.tri == oppooppotri.tri) {
            odprintf("   (Right triangle, wrong orientation)\n");
          }
          odprintf("    First ");
          printtriangle(m, b, &triangleloop);
          odprintf("    Second (nonreciprocating) ");
        ...
        if ((triorg != oppodest) || (tridest != oppoorg)) {
          odprintf("  !! !! Mismatched edge coordinates between two triangles:\n"
                 );
          odprintf("    First mismatched ");
          printtriangle(m, b, &triangleloop);
          odprintf("    Second mismatched ");
          printtriangle(m, b, &oppotri);
          horrors++;
        }
      }
  ...
  if (horrors == 0) {
    if (!b->quiet) {
      odprintf("  In my studied opinion, the mesh appears to be consistent.\n");
    }
  } else if (horrors == 1) {
    odprintf("  !! !! !! !! Precisely one festering wound discovered.\n");
  } else {
    odprintf("  !! !! !! !! %d abominations witnessed.\n", horrors);
  }
...

{
  ...
  b->noexact = 0;
  if (!b->quiet) {
    odprintf("  Checking Delaunay property of mesh...\n");
  }
  horrors = 0;
  /* Run through the list of triangles, checking each one. */
  traversalinit(&m->triangles);
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (shouldbedelaunay) {
        if (nonregular(m, b, triorg, tridest, triapex, oppoapex) > 0.0) {
          if (!b->weighted) {
            odprintf("  !! !! Non-Delaunay pair of triangles:\n");
            odprintf("    First non-Delaunay ");
            printtriangle(m, b, &triangleloop);
            odprintf("    Second non-Delaunay ");
          } else {
            odprintf("  !! !! Non-regular pair of triangles:\n");
            odprintf("    First non-regular ");
            printtriangle(m, b, &triangleloop);
            odprintf("    Second non-regular ");
          }
          printtriangle(m, b, &oppotri);
          horrors++;
        }
  ...
  "  By virtue of my perceptive intelligence, I declare the mesh Delaunay.\n");
    ...
    odprintf("  !! !! !! !! %d obscenities viewed with horror.\n", horrors);
  }
  /* Restore the status of exact arithmetic. */
  b->noexact = saveexact;
}
...

{
  ...
  if (b->verbose > 2) {
    odprintf("  Queueing bad triangle:\n");
    odprintf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
  ...
  exponent = (int)floor(2.0 * exponent + (length > SQUAREROOTTWO));
...

{
  ...
  if (b->verbose) {
    odprintf("    Constructing mapping from vertices to triangles.\n");
  }
  traversalinit(&m->triangles);
  triangleloop.tri = triangletraverse(m);
  while (triangleloop.tri != (triangle *) NULL) {
...

{
  ...
  if (b->verbose > 2) {
    odprintf("  Searching for point (%.12g, %.12g).\n",
  ...
  while (1) {
    if (b->verbose > 2) {
      odprintf("    At (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
...

{
  ...
  if (b->verbose > 2) {
    odprintf("  Randomly sampling for a triangle near point (%.12g, %.12g).\n",
           searchpoint[0], searchpoint[1]);
  }
  ...
  if (b->verbose > 2) {
    odprintf("    Boundary triangle has origin (%.12g, %.12g).\n",
           torg[0], torg[1]);
  }
  ...
  if (m->recenttri.tri != (triangle *) NULL) {
    if (!deadtri(m->recenttri.tri)) {
      ...
      if (dist < searchdist) {
        ...
        searchdist = dist;
        if (b->verbose > 2) {
          odprintf("    Choosing recent triangle with origin (%.12g, %.12g).\n",
                 torg[0], torg[1]);
        }
      }
    }
    ...
    do {
      ...
      if (!deadtri(sampletri.tri)) {
        ...
        if (dist < searchdist) {
          ...
          searchdist = dist;
          if (b->verbose > 2) {
            odprintf("    Choosing triangle with origin (%.12g, %.12g).\n",
                   torg[0], torg[1]);
          }
        }
      }
...

{
  ...
  if (top.tri == m->dummytri) {
    odprintf("Internal error in flip():  Attempt to flip on boundary.\n");
    lnextself(*flipedge);
    return;
  }
  if (m->checksegments) {
    ...
    if (toplsubseg.ss != m->dummysub) {
      odprintf("Internal error in flip():  Attempt to flip a segment.\n");
      lnextself(*flipedge);
      return;
    }
  }
  ...
  if (b->verbose > 2) {
    odprintf("  Edge flip results in left ");
...

{
...
#ifdef SELF_CHECK
  if (top.tri == m->dummytri) {
    odprintf("Internal error in unflip():  Attempt to flip on boundary.\n");
    lnextself(*flipedge);
    return;
  }
  if (m->checksegments) {
    tspivot(*flipedge, toplsubseg);
    if (toplsubseg.ss != m->dummysub) {
      odprintf("Internal error in unflip():  Attempt to flip a subsegment.\n");
      lnextself(*flipedge);
      return;
    }
  }
  ...
  if (b->verbose > 2) {
    odprintf("  Edge unflip results in left ");
    printtriangle(m, b, flipedge);
    odprintf("  and right ");
    printtriangle(m, b, &top);
  }
...

{
  ...
  struct otri newtopright={NULL,-1};
  struct otri botlcasing, botrcasing;
  struct otri toplcasing, toprcasing={NULL,-1};
  struct otri testtri;
  struct osub botlsubseg, botrsubseg;
  struct osub toplsubseg, toprsubseg;
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
      odprintf("Internal error in insertvertex():\n");
      odprintf(
    ...
    if (mirrorflag) {
      if (counterclockwise(m, b, leftvertex, rightvertex, topvertex) < 0.0) {
        odprintf("Internal error in insertvertex():\n");
        odprintf("  Clockwise triangle prior to edge vertex insertion (top).\n");
      }
...
#endif /* SELF_CHECK */
    if (b->verbose > 2) {
      odprintf("  Updating bottom left ");
      printtriangle(m, b, &botright);
      if (mirrorflag) {
        odprintf("  Updating top left ");
        printtriangle(m, b, &topright);
        odprintf("  Creating top right ");
        printtriangle(m, b, &newtopright);
      }
      odprintf("  Creating bottom right ");
    ...
    if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) < 0.0) {
      odprintf("Internal error in insertvertex():\n");
      odprintf("  Clockwise triangle prior to vertex insertion.\n");
    }
    if (counterclockwise(m, b, rightvertex, leftvertex, newvertex) < 0.0) {
      odprintf("Internal error in insertvertex():\n");
      odprintf("  Clockwise triangle after vertex insertion (top).\n");
    }
    if (counterclockwise(m, b, leftvertex, botvertex, newvertex) < 0.0) {
      odprintf("Internal error in insertvertex():\n");
      odprintf("  Clockwise triangle after vertex insertion (left).\n");
    }
    if (counterclockwise(m, b, botvertex, rightvertex, newvertex) < 0.0) {
      odprintf("Internal error in insertvertex():\n");
      odprintf("  Clockwise triangle after vertex insertion (right).\n");
    }
#endif /* SELF_CHECK */
    if (b->verbose > 2) {
      odprintf("  Updating top ");
      printtriangle(m, b, &horiz);
      odprintf("  Creating left ");
      printtriangle(m, b, &newbotleft);
      odprintf("  Creating right ");
      printtriangle(m, b, &newbotright);
    }
  }
        ...
        if (doflip) {
          ...
          if (newvertex != (vertex) NULL) {
            if (counterclockwise(m, b, leftvertex, newvertex, rightvertex) <
                0.0) {
              odprintf("Internal error in insertvertex():\n");
              odprintf("  Clockwise triangle prior to edge flip (bottom).\n");
            }
...
/*
            if (counterclockwise(m, b, rightvertex, farvertex, leftvertex) <
                0.0) {
              odprintf("Internal error in insertvertex():\n");
              odprintf("  Clockwise triangle prior to edge flip (top).\n");
            }
*/
            if (counterclockwise(m, b, farvertex, leftvertex, newvertex) <
                0.0) {
              odprintf("Internal error in insertvertex():\n");
              odprintf("  Clockwise triangle after edge flip (left).\n");
            }
            if (counterclockwise(m, b, newvertex, rightvertex, farvertex) <
                0.0) {
              odprintf("Internal error in insertvertex():\n");
              odprintf("  Clockwise triangle after edge flip (right).\n");
            }
          }
#endif /* SELF_CHECK */
          if (b->verbose > 2) {
...

{
  ...
  if (b->verbose > 2) {
    odprintf("  Triangulating interior polygon at edge\n");
    odprintf("    (%.12g, %.12g) (%.12g, %.12g)\n", leftbasevertex[0],
  ...
  if (b->verbose > 2) {
    odprintf("    Connecting edge to (%.12g, %.12g)\n", bestvertex[0],
           bestvertex[1]);
  }
  if (bestnumber > 1) {
...

{
  ...
  while (!otriequal(*deltri, countingtri)) {
    ...
    if (countingtri.tri == m->dummytri) {
      odprintf("Internal error in deletevertex():\n");
      odprintf("  Attempt to delete boundary vertex.\n");
      internalerror();
    }
#endif /* SELF_CHECK */
    edgecount++;
...

{
  ...
  if (b->verbose > 2) {
    odprintf("  Creating base bounding ");
  ...
  while (1) {
    ...
    if (leftfinished && rightfinished) {
      ...
      if (b->verbose > 2) {
        odprintf("  Creating top bounding ");
...

{
  ...
  if (b->verbose > 2) {
    odprintf("  Triangulating %d vertices.\n", vertices);
  }
  if (vertices == 2) {
    ...
    divconqrecurse(m, b, &sortarray[divider], vertices - divider, 1 - axis,
                   &innerright, farright);
    if (b->verbose > 1) {
      odprintf("  Joining triangulations with %d and %d vertices.\n", divider,
             vertices - divider);
    }
...

{
  ...
  if (b->verbose) {
    odprintf("  Removing ghost triangles.\n");
  }
...
"Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored.\n",
  ...
  if (b->verbose) {
    odprintf("  Forming triangulation.\n");
  }
...

{
  ...
  if (b->verbose) {
    odprintf("  Creating triangular bounding box.\n");
  }
...

{
  ...
  if (b->verbose) {
    odprintf("  Removing triangular bounding box.\n");
  }
  /* Find a boundary triangle. */
...

{
  ...
  boundingbox(m, b);
  if (b->verbose) {
    odprintf("  Incrementally inserting vertices.\n");
  }
  traversalinit(&m->vertices);
...

{
  ...
  struct otri searchtri;
  struct otri fliptri;
  struct otri lefttri, righttri, farlefttri={NULL,-1}, farrighttri={NULL,-1};
  ...
  if (b->verbose) {
    odprintf("  Placing vertices in event heap.\n");
  }
  ...
  do {
    if (heapsize == 0) {
      odprintf("Error:  Input vertices are all identical.\n");
	  throw new std::runtime_error("Could not triangulate domain:\nInput vertices are all identical.");
      //triexit(1);
    }
...
#else /* not REDUCED */
  if (!b->quiet) {
    odprintf("Constructing Delaunay triangulation ");
    if (b->incremental) {
      odprintf("by incremental method.\n");
    } else if (b->sweepline) {
      odprintf("by sweepline method.\n");
    } else {
      odprintf("by divide-and-conquer method.\n");
    }
  }
  if (b->incremental) {
...

{
  ...
  incorners = corners;
  if (incorners < 3) {
    odprintf("Error:  Triangles must have at least 3 vertices.\n");
  ...
  if (elefile == (FILE *) NULL) {
    odprintf("  Error:  Cannot access file %s.\n", elefilename);
    triexit(1);
  }
  ...
  if (*stringptr == '\0') {
    ...
    if (incorners < 3) {
      odprintf("Error:  Triangles in %s must have at least 3 vertices.\n",
             elefilename);
      triexit(1);
    }
  }
  ...
  if (b->vararea) {
    ...
    if (areaelements != m->inelements) {
      odprintf("Error:  %s and %s disagree on number of triangles.\n",
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < b->firstnumber) ||
        ...
        odprintf("Error:  Triangle %ld has an invalid vertex index.\n",
               elementnumber);
		throw new std::runtime_error("Could not triangulate domain:\nTriangle has an invalid vertex index.");
        //triexit(1);
      }
    }
#else /* not TRILIBRARY */
    ...
    for (j = 0; j < 3; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        odprintf("Error:  Triangle %ld is missing vertex %d in %s.\n",
  ...
  if (b->poly) {
    if (b->verbose) {
      odprintf("  Marking segments in triangulation.\n");
    }
    ...
    while (subsegloop.ss != (subseg *) NULL) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        odprintf("Error:  Segment %ld has no endpoints in %s.\n", segmentnumber,
               polyfilename);
        triexit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        odprintf("Error:  Segment %ld is missing its second endpoint in %s.\n",
      ...
      for (j = 0; j < 2; j++) {
        if ((end[j] < b->firstnumber) ||
          ...
          odprintf("Error:  Segment %ld has an invalid vertex index.\n", 
                 segmentnumber);
		  throw new std::runtime_error("Could not triangulate domain:\nSegment has an invalid vertex index.");
          //triexit(1);
        }
      }
...

{
  ...
  while (leftflag) {
    ...
    onextself(*searchtri);
    if (searchtri->tri == m->dummytri) {
      odprintf("Internal error in finddirection():  Unable to find a\n");
      odprintf("  triangle leading from (%.12g, %.12g) to", startvertex[0],
             startvertex[1]);
      odprintf("  (%.12g, %.12g).\n", searchpoint[0], searchpoint[1]);
      internalerror();
    }
...

{
  ...
  denom = ty * ex - tx * ey;
  if (denom == 0.0) {
    odprintf("Internal error in segmentintersection():");
    odprintf("  Attempt to find intersection of parallel segments.\n");
    internalerror();
  }
  ...
  if (success != SUCCESSFULVERTEX) {
    odprintf("Internal error in segmentintersection():\n");
    odprintf("  Failure to split a segment.\n");
    internalerror();
  }
  ...
  if ((leftvertex[0] == endpoint1[0]) && (leftvertex[1] == endpoint1[1])) {
    ...
    odprintf("Internal error in segmentintersection():\n");
    odprintf("  Topological inconsistency after splitting a segment.\n");
    internalerror();
  }
...

{
  ...
  if (b->verbose > 2) {
    odprintf("Forcing segment into triangulation by recursive splitting:\n");
  ...
  if (success == DUPLICATEVERTEX) {
    if (b->verbose > 2) {
      odprintf("  Segment intersects existing vertex (%.12g, %.12g).\n",
             newvertex[0], newvertex[1]);
    }
    ...
    if (success == VIOLATINGVERTEX) {
      if (b->verbose > 2) {
        odprintf("  Two segments intersect at (%.12g, %.12g).\n",
               newvertex[0], newvertex[1]);
      }
      ...
      success = insertvertex(m, b, newvertex, &searchtri1, &brokensubseg,
                             0, 0);
      if (success != SUCCESSFULVERTEX) {
        odprintf("Internal error in conformingedge():\n");
        odprintf("  Failure to split a segment.\n");
        internalerror();
      }
    }
...
{
  struct otri searchtri1={NULL,-1}, searchtri2={NULL,-1};
  triangle encodedtri;
  vertex checkvertex;
  triangle ptr;                         /* Temporary variable used by sym(). */

  if (b->verbose > 1) {
    odprintf("  Connecting (%.12g, %.12g) to (%.12g, %.12g).\n",
  ...
  if (checkvertex != endpoint1) {
    ...
    if (locate(m, b, endpoint1, &searchtri1) != ONVERTEX) {
      ...
      odprintf("  (%.12g, %.12g) in triangulation.\n",
             endpoint1[0], endpoint1[1]);
      internalerror();
    }
  }
...

{
  ...
  if (b->poly) {
    if (!b->quiet) {
      odprintf("Recovering segments in Delaunay triangulation.\n");
    }
#ifdef TRILIBRARY
    ...
    if (m->insegments > 0) {
      makevertexmap(m, b);
      if (b->verbose) {
        odprintf("  Recovering PSLG segments.\n");
      }
    }
    ...
    for (i = 0; i < m->insegments; i++) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        odprintf("Error:  Segment %d has no endpoints in %s.\n",
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        odprintf("Error:  Segment %d is missing its second endpoint in %s.\n",
               b->firstnumber + i, polyfilename);
        triexit(1);
      } else {
      ...
      if ((end1 < b->firstnumber) ||
        ...
        if (!b->quiet) {
          odprintf("Warning:  Invalid first endpoint of segment %d in %s.\n",
                 b->firstnumber + i, polyfilename);
        }
      } else if ((end2 < b->firstnumber) ||
                 (end2 >= b->firstnumber + m->invertices)) {
        if (!b->quiet) {
          odprintf("Warning:  Invalid second endpoint of segment %d in %s.\n",
                 b->firstnumber + i, polyfilename);
        }
      } else {
        ...
        if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
          if (!b->quiet) {
            odprintf("Warning:  Endpoints of segment %d are coincident in %s.\n",
                   b->firstnumber + i, polyfilename);
          }
        } else {
  ...
  if (b->convex || !b->poly) {
    ...
    if (b->verbose) {
      odprintf("  Enclosing convex hull with segments.\n");
    }
    markhull(m, b);
  }
}
...

{
  ...
  if (b->verbose) {
    odprintf("  Marking concavities (external triangles) for elimination.\n");
  }
  /* Find a triangle handle on the hull. */
  hulltri.tri = m->dummytri;
  hulltri.orient = 0;
...

{
  ...
  if (b->verbose) {
    odprintf("  Marking neighbors of marked triangles.\n");
  }
  ...
  while (virusloop != (triangle **) NULL) {
    ...
    if (b->verbose > 2) {
      ...
      odprintf("    Checking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
  ...
  if (b->verbose) {
    odprintf("  Deleting marked triangles.\n");
  }
  ...
  while (virusloop != (triangle **) NULL) {
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if (testvertex != (vertex) NULL) {
        ...
        if (killorg) {
          if (b->verbose > 1) {
            odprintf("    Deleting vertex (%.12g, %.12g)\n",
...

{
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if ((neighbor.tri != m->dummytri) && !infected(neighbor)
        ...
        if (b->verbose > 2) {
          ...
          odprintf("    Marking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
                 regionorg[0], regionorg[1], regiondest[0], regiondest[1],
                 regionapex[0], regionapex[1]);
        }
        /* Infect the neighbor. */
  ...
  if (b->verbose > 1) {
    odprintf("  Unmarking marked triangles.\n");
  }
  traversalinit(&m->viri);
  virusloop = (triangle **) traverse(&m->viri);
  while (virusloop != (triangle **) NULL) {
...

{
  ...
  if (!(b->quiet || (b->noholes && b->convex))) {
    odprintf("Removing unwanted triangles.\n");
    if (b->verbose && (holes > 0)) {
      odprintf("  Marking holes for elimination.\n");
    }
  }
  ...
  if (regions > 0) {
    if (!b->quiet) {
      if (b->regionattrib) {
        if (b->vararea) {
          odprintf("Spreading regional attributes and area constraints.\n");
        } else {
          odprintf("Spreading regional attributes.\n");
        }
      } else { 
        odprintf("Spreading regional area constraints.\n");
      }
    }
...
void precisionerror()
{
  odprintf("Try increasing the area criterion and/or reducing the minimum\n");
  odprintf("  allowable angle so that tiny triangles are not created.\n");
#ifdef SINGLE
  odprintf("Alternatively, try recompiling me with double precision\n");
  odprintf("  arithmetic (by removing \"#define SINGLE\" from the\n");
  odprintf("  source file or \"-DSINGLE\" from the makefile).\n");
#endif /* SINGLE */
}
...

{
  ...
  "  Splitting subsegment (%.12g, %.12g) (%.12g, %.12g) at (%.12g, %.12g).\n",
        ...
        if (((newvertex[0] == eorg[0]) && (newvertex[1] == eorg[1])) ||
          ...
          odprintf("Error:  Ran out of precision at (%.12g, %.12g).\n",
                 newvertex[0], newvertex[1]);
          odprintf("I attempted to split a segment to a smaller size than\n");
          odprintf("  can be accommodated by the finite precision of\n");
          odprintf("  floating point arithmetic.\n");
          precisionerror();
        ...
        if ((success != SUCCESSFULVERTEX) && (success != ENCROACHINGVERTEX)) {
          odprintf("Internal error in splitencsegs():\n");
          odprintf("  Failure to split a segment.\n");
          internalerror();
        }
        if (m->steinerleft > 0) {
...

{
  ...
  if (b->verbose) {
    odprintf("  Making a list of bad triangles.\n");
  }
  traversalinit(&m->triangles);
  triangleloop.orient = 0;
  triangleloop.tri = triangletraverse(m);
...

{
  ...
  if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
    ...
    if (b->verbose > 1) {
      odprintf("  Splitting this triangle at its circumcenter:\n");
      odprintf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", borg[0],
    ...
    if (errorflag) {
      if (b->verbose) {
        odprintf("  The new vertex is at the circumcenter of triangle\n");
        odprintf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
               borg[0], borg[1], bdest[0], bdest[1], bapex[0], bapex[1]);
      }
      odprintf("This probably means that I am trying to refine triangles\n");
      odprintf("  to a smaller size than can be accommodated by the finite\n");
      odprintf("  precision of floating point arithmetic.  (You can be\n");
      odprintf("  sure of this if I fail to terminate.)\n");
      precisionerror();
    }
  }
}
...

{
  ...
  if (!b->quiet) {
    odprintf("Adding Steiner points to enforce quality.\n");
  }
  /* Initialize the pool of encroached subsegments. */
  poolinit(&m->badsubsegs, sizeof(struct badsubseg), BADSUBSEGPERBLOCK,
           BADSUBSEGPERBLOCK, 0);
  if (b->verbose) {
    odprintf("  Looking for encroached subsegments.\n");
  }
  /* Test all segments to see if they're encroached. */
  tallyencs(m, b);
  if (b->verbose && (m->badsubsegs.items > 0)) {
    odprintf("  Splitting encroached subsegments.\n");
  }
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
    ...
    m->checkquality = 1;
    if (b->verbose) {
      odprintf("  Splitting bad triangles.\n");
    }
  ...
  if (!b->quiet && b->conformdel && (m->badsubsegs.items > 0) &&
    ...
    if (m->badsubsegs.items == 1) {
      odprintf("  one encroached subsegment, and therefore might not be truly\n"
             );
    } else {
      odprintf("  %ld encroached subsegments, and therefore might not be truly\n"
             , m->badsubsegs.items);
    }
    odprintf("  Delaunay.  If the Delaunay property is important to you,\n");
    odprintf("  try increasing the number of Steiner points (controlled by\n");
    odprintf("  the -S switch) slightly and try again.\n\n");
  }
}
...

{
  ...
  if (!b->quiet) {
    odprintf("Adding vertices for second-order triangles.\n");
  }
...

{
  ...
  do {
    ...
    if (result == (char *) NULL) {
      odprintf("  Error:  Unexpected end of file in %s.\n", infilename);
      triexit(1);
    }
...

{
  ...
  if (m->invertices < 3) {
    odprintf("Error:  Input must have at least three input vertices.\n");
    triexit(1);
  }
  if (m->mesh_dim != 2) {
    odprintf("Error:  Triangle only works with two-dimensional meshes.\n");
    triexit(1);
  }
  if (m->nextras == 0) {
    b->weighted = 0;
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      odprintf("Error:  Vertex %d has no x coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
    x = (REAL) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      odprintf("Error:  Vertex %d has no y coordinate.\n", b->firstnumber + i);
      triexit(1);
    }
...

{
  ...
  if (m->invertices < 3) {
    odprintf("Error:  Input must have at least three input vertices.\n");
	throw new std::runtime_error("Could not triangulate domain:\nInput must have at least three input vertices.");
    //triexit(1);
  }
  if (m->nextras == 0) {
    b->weighted = 0;
  ...
  if (*holes > 0) {
    ...
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        odprintf("Error:  Hole %d has no x coordinate.\n",
               b->firstnumber + (i >> 1));
        triexit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        odprintf("Error:  Hole %d has no y coordinate.\n",
               b->firstnumber + (i >> 1));
        triexit(1);
      } else {
    ...
    if (*regions > 0) {
      ...
      *rlist = regionlist;
      ...
      for (i = 0; i < *regions; i++) {
        ...
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          odprintf("Error:  Region %d has no x coordinate.\n",
                 b->firstnumber + i);
          triexit(1);
        } else {
        ...
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          odprintf("Error:  Region %d has no y coordinate.\n",
                 b->firstnumber + i);
          triexit(1);
        } else {
          regionlist[index++] = (REAL) strtod(stringptr, &stringptr);
...

{
  ...
  if (outfile == (FILE *) NULL) {
    odprintf("  Error:  Cannot create file %s.\n", nodefilename);
    triexit(1);
  }
...
#ifdef TRILIBRARY
  if (!b->quiet) {
    odprintf("Writing Voronoi vertices.\n");
  }
...
#ifdef TRILIBRARY
  if (!b->quiet) {
    odprintf("Writing Voronoi edges.\n");
  }
...

{
  ...
  odprintf("Mesh quality statistics:\n\n");
  radconst = PI / 18.0;
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++) {
  ...
  odprintf("  Smallest area: %16.5g   |  Largest area: %16.5g\n",
         smallestarea, biggestarea);
  odprintf("  Shortest edge: %16.5g   |  Longest edge: %16.5g\n",
         shortest, longest);
  odprintf("  Shortest altitude: %12.5g   |  Largest aspect ratio: %8.5g\n\n",
         minaltitude, worstaspect);

  odprintf("  Triangle aspect ratio histogram:\n");
  odprintf("  1.1547 - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
         ratiotable[0], aspecttable[0], ratiotable[7], ratiotable[8],
         aspecttable[8]);
  for (i = 1; i < 7; i++) {
    odprintf("  %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
           ratiotable[i - 1], ratiotable[i], aspecttable[i],
           ratiotable[i + 7], ratiotable[i + 8], aspecttable[i + 8]);
  }
  odprintf("  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n",
         ratiotable[6], ratiotable[7], aspecttable[7], ratiotable[14],
         aspecttable[15]);
  odprintf("  (Aspect ratio is longest edge divided by shortest altitude)\n\n");

  odprintf("  Smallest angle: %15.5g   |  Largest angle: %15.5g\n\n",
         smallestangle, biggestangle);

  odprintf("  Angle histogram:\n");
  for (i = 0; i < 9; i++) {
    odprintf("    %3d - %3d degrees:  %8d    |    %3d - %3d degrees:  %8d\n",
...
{
  odprintf("\nStatistics:\n\n");
  odprintf("  Input vertices: %d\n", m->invertices);
  if (b->refine) {
    odprintf("  Input triangles: %d\n", m->inelements);
  }
  if (b->poly) {
    odprintf("  Input segments: %d\n", m->insegments);
    if (!b->refine) {
      odprintf("  Input holes: %d\n", m->holes);
    }
  }

  odprintf("\n  Mesh vertices: %ld\n", m->vertices.items - m->undeads);
  odprintf("  Mesh triangles: %ld\n", m->triangles.items);
  odprintf("  Mesh edges: %ld\n", m->edges);
  odprintf("  Mesh exterior boundary edges: %ld\n", m->hullsize);
  if (b->poly || b->refine) {
    odprintf("  Mesh interior boundary edges: %ld\n",
           m->subsegs.items - m->hullsize);
    odprintf("  Mesh subsegments (constrained edges): %ld\n",
           m->subsegs.items);
  }
  odprintf("\n");
  ...
  if (b->verbose) {
    ...
    odprintf("Memory allocation statistics:\n\n");
    odprintf("  Maximum number of vertices: %ld\n", m->vertices.maxitems);
    odprintf("  Maximum number of triangles: %ld\n", m->triangles.maxitems);
    if (m->subsegs.maxitems > 0) {
      odprintf("  Maximum number of subsegments: %ld\n", m->subsegs.maxitems);
    }
    if (m->viri.maxitems > 0) {
      odprintf("  Maximum number of viri: %ld\n", m->viri.maxitems);
    }
    if (m->badsubsegs.maxitems > 0) {
      odprintf("  Maximum number of encroached subsegments: %ld\n",
             m->badsubsegs.maxitems);
    }
    if (m->badtriangles.maxitems > 0) {
      odprintf("  Maximum number of bad triangles: %ld\n",
             m->badtriangles.maxitems);
    }
    if (m->flipstackers.maxitems > 0) {
      odprintf("  Maximum number of stacked triangle flips: %ld\n",
             m->flipstackers.maxitems);
    }
    if (m->splaynodes.maxitems > 0) {
      odprintf("  Maximum number of splay tree nodes: %ld\n",
             m->splaynodes.maxitems);
    }
    odprintf("  Approximate heap memory use (bytes): %ld\n\n",
    ...
    odprintf("Algorithmic statistics:\n\n");
    if (!b->weighted) {
      odprintf("  Number of incircle tests: %ld\n", m->incirclecount);
    } else {
      odprintf("  Number of 3D orientation tests: %ld\n", m->orient3dcount);
    }
    odprintf("  Number of 2D orientation tests: %ld\n", m->counterclockcount);
    if (m->hyperbolacount > 0) {
      odprintf("  Number of right-of-hyperbola tests: %ld\n",
             m->hyperbolacount);
    }
    if (m->circletopcount > 0) {
      odprintf("  Number of circle top computations: %ld\n",
             m->circletopcount);
    }
    if (m->circumcentercount > 0) {
      odprintf("  Number of triangle circumcenter computations: %ld\n",
             m->circumcentercount);
    }
    odprintf("\n");
  }
...

{
  ...
  if (!b.quiet) {
    gettimeofday(&tv2, &tz);
    if (b.refine) {
      odprintf("Mesh reconstruction");
    } else {
      odprintf("Delaunay");
    }
    odprintf(" milliseconds:  %ld\n", 1000l * (tv2.tv_sec - tv1.tv_sec) +
  ...
  if (!b.quiet) {
    ...
    if (b.usesegments && !b.refine) {
      odprintf("Segment milliseconds:  %ld\n",
...
#ifndef CDT_ONLY
    if (b.quality) {
      odprintf("Quality milliseconds:  %ld\n",
  ...
  if (b.nonodewritten || (b.noiterationnum && m.readnodefile)) {
    if (!b.quiet) {
#ifdef TRILIBRARY
      odprintf("NOT writing vertices.\n");
#else /* not TRILIBRARY */
      odprintf("NOT writing a .node file.\n");
#endif /* not TRILIBRARY */
    }
  ...
  if (b.noelewritten) {
    if (!b.quiet) {
#ifdef TRILIBRARY
      odprintf("NOT writing triangles.\n");
#else /* not TRILIBRARY */
      odprintf("NOT writing an .ele file.\n");
#endif /* not TRILIBRARY */
    }
  } else {
#ifdef TRILIBRARY
  ...
  if (b.poly || b.convex) {
    ...
    if (b.nopolywritten || b.noiterationnum) {
      if (!b.quiet) {
#ifdef TRILIBRARY
        odprintf("NOT writing segments.\n");
#else /* not TRILIBRARY */
        odprintf("NOT writing a .poly file.\n");
#endif /* not TRILIBRARY */
      }
    } else {
#ifdef TRILIBRARY
  ...
  if (!b.quiet) {
    ...
    odprintf("Total running milliseconds:  %ld\n",
